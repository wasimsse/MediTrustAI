...
# Base directories
INPUT_DIR = r'C:\Users\galaxy\Documents\VsCode\CodeTrustedAI\files_medhistory\input'
...
def create_batch_job(file_id):
    batch_response = client.batches.create(
        ...
        metadata={"description": "history batch job"}
    )
...
def estimate_completion_time(num_prompts):
    # Rough estimate: 2 minutes per prompt
    estimated_minutes = num_prompts * 2
...
def process_file(jsonl_file_path):
    ...
    try:
        # Count prompts and estimate completion time
        num_prompts = count_prompts(jsonl_file_path)
        estimated_time = estimate_completion_time(num_prompts)
        log_entry.update({
...
def main():
    ...
    with tqdm(total=len(jsonl_file_paths), desc="Processing files", unit="file") as pbar:
