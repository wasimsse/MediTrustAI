...
prompt1 = " Allan Mask email address is allan.mask@example.com, birthdate is 2021.12.25 has phone number +358 409876541."
prompt1_analyzer_result = analyzer.analyze(text=prompt1, language='en')

print(f"\n\n Analyzer results: {prompt1_analyzer_result} ")
...
# Create custom operator for PERSON, phone_number, email, date_of_birth entity
...
# invoke the anonymize funtion with the text
# Operators to get the anonymization output:
prompt1_anon_result = engine.anonymize(text=prompt1, analyzer_results=prompt1_analyzer_result, operators=operators)

print(f"\n\n Original text: {prompt1}")
print(f"\n\n anonymize1 results: {prompt1_anon_result.text}")
...
analyzer_results2 = analyzer.analyze(text=prompt1, entities=["EMAIL_ADDRESS", "PERSON", "DATE_TIME"], language="en")

print(f"\n\n Original Text: \n{prompt1}")
print(f"\n\n Analyzer Results: \n{analyzer_results2}")

# Anonymize the text and analysis results
anonymized_text2, entity_mapping2 = anonymize_text(analyzer_results2, prompt1)
